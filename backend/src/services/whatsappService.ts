...
export const DEFAULT_ALERT_PHONE = process.env.ALERT_WHATSAPP_NUMBER || '+91 7827392589';
 ...
 */
export function formatLateAttendanceMessage(employee: LateEmployee, dateStr: string): string {
 ...
 */
export async function dispatchWhatsApp(toPhone: string, message: string): Promise<{ success: boolean; provider: string; response?: any }> {
 ...
 */
export async function sendLateAttendanceAlert(employee: LateEmployee, dateStr: string): Promise<boolean> {
  const alertType = 'LATE_ATTENDANCE_11AM';
  const phone = DEFAULT_ALERT_PHONE;

  // 1. Idempotency Check: Don't send twice for the same employee on the same date
  const checkRes = await query(
    `SELECT id FROM whatsapp_logs WHERE employee_id = $1 AND alert_type = $2 AND attendance_date = $3`,
    [employee.id, alertType, dateStr]
  );
 ...
 */
export async function checkAndSendLateAttendanceAlerts(overrideDateStr?: string) {
  const now = new Date();
  ...
  const holidayRes = await query(
    `SELECT id, name FROM holidays WHERE holiday_date = $1 AND is_active = true`,
    [dateStr]
  );
  ...
  const activeEmployees: LateEmployee[] = employeesRes.rows;
  let alertsSent = 0;
  const skippedList: { name: string; reason: string }[] = [];
  const alertedList: { name: string; employeeId: string }[] = [];
