...
class PayrollCalculationService {
    ...
    static async getSettings() {
        const res = await (0, db_1.query)('SELECT * FROM payroll_settings WHERE id = 1');
     ...
     */
    static async calculateWorkingDays(year, month, settings) {
        ...
        const holRes = await (0, db_1.query)(`SELECT holiday_date FROM holidays WHERE is_active = true AND holiday_date >= $1 AND holiday_date <= $2`, [startDateStr, endDateStr]);
        const holidaySet = new Set(holRes.rows.map((r) => new Date(r.holiday_date).toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' })));
     ...
     */
    static async calculateCyclePreview(year, month, manualAdjustments = {}) {
    ...
    `);
        ...
        const [attRes, attSettings, holRes, leaveRes] = await Promise.all([
            ...
            (0, db_1.query)(`SELECT holiday_date FROM holidays WHERE is_active = true AND holiday_date >= $1 AND holiday_date <= $2`, [startDateStr, actualEndStr]),
            (0, db_1.query)(`SELECT * FROM leave_requests WHERE status = 'APPROVED' AND from_date <= $2 AND to_date >= $1`, [startDateStr, actualEndStr]),
        ]);
        ...
        const holSet = new Set(holRes.rows.map((r) => new Date(r.holiday_date).toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' })));
     ...
     */
    static async checkSyncStatus(cycleId) {
        ...
        const itemsRes = await (0, db_1.query)('SELECT * FROM payroll_items WHERE cycle_id = $1', [cycleId]);
        const mismatches = [];
        for (const item of itemsRes.rows) {
            ...
            if (fresh) {
                ...
                if (Math.abs(storedPayable - freshPayable) > 0.01 ||
        ...
        if (isOutOfSync && cycle.status === 'FINALIZED') {
            await (0, db_1.query)(`UPDATE payroll_cycles SET status = 'OUT_OF_SYNC', updated_at = NOW() WHERE id = $1`, [cycleId]);
        }
