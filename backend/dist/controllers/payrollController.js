...
const payrollCalculationService_1 = require("../services/payrollCalculationService");
const salarySlipService_1 = require("../services/salarySlipService");
const payrollReportService_1 = require("../services/payrollReportService");
...
class PayrollController {
    ...
    static async getSettings(req, res) {
        try {
            const settings = await payrollCalculationService_1.PayrollCalculationService.getSettings();
    ...
    static async updateSettings(req, res) {
        try {
            const { calculation_method, fixed_working_days, weekly_offs, late_deduction_rule, pf_enabled, pf_employee_percent, pf_employer_percent, pf_wage_ceiling, esic_enabled, esic_employee_percent, esic_employer_percent, esic_wage_limit, pt_enabled, default_pt_amount, } = req.body;
            await (0, db_1.query)(`
      ...
      `, [
            ...
            const updated = await payrollCalculationService_1.PayrollCalculationService.getSettings();
     ...
     */
    static async getSalaryProfiles(req, res) {
        try {
     ...
     */
    static async updateSalaryProfile(req, res) {
        try {
            const employeeId = parseInt(req.params.employeeId);
            const { monthly_ctc, basic_salary, hra, da, conveyance_allowance, medical_allowance, special_allowance, pf_applicable, esic_applicable, pt_applicable, tds_percent, bank_name, account_number, ifsc_code, pan_number, uan_number, esic_number, effective_date, revision_reason, } = req.body;
            // Check existing profile for audit revision
            const existing = await (0, db_1.query)('SELECT * FROM employee_salary_profiles WHERE employee_id = $1', [employeeId]);
     ...
     */
    static async getSalaryRevisions(req, res) {
        try {
            const employeeId = parseInt(req.params.employeeId);
            const result = await (0, db_1.query)(`
     ...
     */
    static async getCycleDetails(req, res) {
        try {
      ...
      `, [cycle.id]);
            // Check sync status
            const syncInfo = await payrollCalculationService_1.PayrollCalculationService.checkSyncStatus(cycle.id);
            res.json({
                success: true,
                isFinalized: true,
                cycle,
    ...
    static async calculatePreview(req, res) {
        try {
            ...
            const preview = await payrollCalculationService_1.PayrollCalculationService.calculateCyclePreview(y, m, adjustments || {});
     ...
     */
    static async finalizeCycle(req, res) {
        try {
            const { year, month, adjustments } = req.body;
            ...
            const cycleRes = await (0, db_1.query)(`
      ...
      `, [
            ...
            for (const item of preview.items) {
                ...
                const profRes = await (0, db_1.query)('SELECT * FROM employee_salary_profiles WHERE employee_id = $1', [item.employee_id]);
                const prof = profRes.rows[0];
                // Generate PDF slip
                const pdfUrl = await salarySlipService_1.SalarySlipService.generateSalarySlipPDF(item, y, m, prof);
                await (0, db_1.query)(`
        ...
        `, [
                ...
                try {
                    await notificationService_1.NotificationService.notifyUser(item.employee_id, {
    ...
    static async unlockCycle(req, res) {
        try {
            ...
            await (0, db_1.query)(`UPDATE payroll_cycles SET status = 'DRAFT', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [cycleId]);
     ...
     */
    static async updatePayrollItem(req, res) {
        try {
            const itemId = parseInt(req.params.id);
            const { bonus, incentive, overtime_pay, advance_deduction, other_deductions, notes } = req.body;
            const itemRes = await (0, db_1.query)('SELECT * FROM payroll_items WHERE id = $1', [itemId]);
            ...
            const cycleRes = await (0, db_1.query)('SELECT * FROM payroll_cycles WHERE id = $1', [item.cycle_id]);
     ...
     */
    static async exportSalaryRegister(req, res) {
        try {
            const cycleId = req.query.cycleId ? parseInt(req.query.cycleId) : undefined;
            const month = req.query.month ? parseInt(req.query.month) : undefined;
            const year = req.query.year ? parseInt(req.query.year) : undefined;
            await payrollReportService_1.PayrollReportService.exportSalaryRegisterExcel({ cycleId, month, year }, res);
        }
        catch (err) {
    ...
    static async exportPFReport(req, res) {
        try {
            const cycleId = req.query.cycleId ? parseInt(req.query.cycleId) : undefined;
            const month = req.query.month ? parseInt(req.query.month) : undefined;
            const year = req.query.year ? parseInt(req.query.year) : undefined;
            await payrollReportService_1.PayrollReportService.exportPFReportExcel({ cycleId, month, year }, res);
        }
        catch (err) {
    ...
    static async exportESICReport(req, res) {
        try {
            const cycleId = req.query.cycleId ? parseInt(req.query.cycleId) : undefined;
            const month = req.query.month ? parseInt(req.query.month) : undefined;
            const year = req.query.year ? parseInt(req.query.year) : undefined;
            await payrollReportService_1.PayrollReportService.exportESICReportExcel({ cycleId, month, year }, res);
        }
        catch (err) {
    ...
    static async exportSummaryPDF(req, res) {
        try {
            const cycleId = req.query.cycleId ? parseInt(req.query.cycleId) : undefined;
            const month = req.query.month ? parseInt(req.query.month) : undefined;
            const year = req.query.year ? parseInt(req.query.year) : undefined;
            await payrollReportService_1.PayrollReportService.exportPayrollSummaryPDF({ cycleId, month, year }, res);
        }
        catch (err) {
